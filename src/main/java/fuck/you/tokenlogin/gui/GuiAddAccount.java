...
public class GuiAddAccount extends GuiScreen
{
	...
	public void initGui( )
	{
		...
		this.buttonList.add( add = new GuiButton( 777, midwidth - 100, this.clientToken.y + this.clientToken.height + 5, 98, 20, "Add" ) );
	...
	public void drawScreen( int mouseX, int mouseY, float partialTicks )
	{
		...
		this.drawCenteredString( this.fontRenderer, "Add Account", midwidth, this.height / 4, -1 );
		...
		if( error )
		{
			this.drawCenteredString( this.fontRenderer, "Account with specified username is already in the list",
					midwidth, add.y + add.height + 5, java.awt.Color.red.getRGB( ) );
		}
	}
	...
	protected void actionPerformed( GuiButton button )
	{
		...
		else if( button.id == 777 ) // add account
		{
			if( add.enabled )
			{
				Account account = new Account( );
				account.username = username.getText( );
				account.accessToken = accessToken.getText( );
				account.clientToken = clientToken.getText( );
